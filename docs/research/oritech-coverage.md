...
| **Vulcanus: lava and calcite** — lava as infinite | `planned` | Pump (only water counts as infinite) | **Core** — the Offshore Pump, ruled Factorio-faithful | **Ruling (2026-09-11):** lava is pumped by the Offshore Pump, which is Factorio's own entity on Vulcanus, unpowered and sited against the lake. Oritech's Pump is not used for lava. Its infinite rule is two hard-coded `isSame(Fluids.WATER)` checks (`PumpBlockEntity:119`, `:344`), and making lava infinite there would take a mixin plus the power draw Factorio's pump does not have. The core change is small, because **siting is already fluid-agnostic**. `OffshorePumpBlock.neighboursOf` tests only `FluidState.isSource`, so a pump beside lava is accepted today. But `OffshorePumpBlockEntity.serverTick` always fills `Fluids.WATER`. So the pump has to record which source fluid it was sited on and fill with that, at the corpus rate for lava. **This is a latent defect now, not only a Vulcanus task:** a pump placed against a lava source (Ignus) or an Oritech oil source makes water from it. ADR-0050 names water, so lava is an amendment. The same fix decides whether oil is refused at siting, which it has to be, or finite oil stops being finite (see the pumpjack row). |
...
- **Java on Oritech** — assembling machines, chemical plant, centrifuge, electric furnace, oil processing, the electric drill (on `DeepDrillEntity`), steam engine, boiler (optional), solar, pole reach, module tiers 2–3, productivity, beacon, laser turret, equipment grid, the drone port across dimensions, the agricultural tower, the Vulcanus foundry, and the reactor as a steam source.
